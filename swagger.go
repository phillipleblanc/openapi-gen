...
import (
	...
	"github.com/phillipleblanc/openapi-gen/models"
)
...
type Swagger struct {
	...
	Entities map[string]map[string][]models.Resource
}
...
func parse(insomnia models.Insomnia) map[string]map[string][]models.Resource {
	groupNames = make(map[string]string)
	entities := make(map[string]map[string][]models.Resource)
	for _, resource := range insomnia.Resources {
		if resource.Type == REQUEST_GROUP {
			groupNames[resource.ID] = resource.Name
			entities[resource.ID] = make(map[string][]models.Resource)
		}
...
func readInsomniaExport(fileName string) models.Insomnia {
	raw, err := ioutil.ReadFile(fileName)
	if err != nil {
		log.Fatal(err)
	}
	var insomnia models.Insomnia
...
func fetchVariables(resource *models.Resource) {
	re := regexp.MustCompile("/{(.*?)}#*")
